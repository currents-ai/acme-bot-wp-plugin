 ...
 *
 * @package    AcmeBot
 * @subpackage AcmeBot/admin/partials
 */
...
$documentation_url = 'https://acme.bot/docs/application-passwords';
$logo_url = 'https://acme.bot/logo/logo-small-wide.svg';
?>
<div class="wrap settings-wrap">
    <h1 class="acmebot-title"><?php echo esc_html(get_admin_page_title()); ?></h1>
        ...
        <div class="acmebot-card">
            <div class="acmebot-card-body">
                <?php if ($is_connected) : ?>
                    ...
                    <h2 class="acmebot-card-title"><?php esc_html_e('Connection Active', 'acme-bot'); ?></h2>
                    <div class="acmebot-connected-content">
                        ...
                        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="acmebot-connect-form">
                            ...
                            <div class="acmebot-btn-center">
                                <?php submit_button(__('Reconnect to Acme Bot', 'acme-bot'), 'primary large acmebot-btn', 'submit', true); ?>
                            </div>
                        </form>
                        <p class="acmebot-muted"><?php esc_html_e('If you need to refresh or update your connection, click "Reconnect".', 'acme-bot'); ?></p>
                    </div>
                <?php else : ?>
                    <div class="acmebot-logo-box">
                        <img src="<?php echo esc_url($logo_url); ?>" alt="Acme Bot" class="acmebot-logo-img" />
                    ...
                    <h2 class="acmebot-card-title"><?php esc_html_e('Connect to Acme Bot', 'acme-bot'); ?></h2>
                    <p><?php esc_html_e('Set up your Acme Bot account to enable AI features on this site.', 'acme-bot'); ?></p>
                    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="acmebot-connect-form">
                        <input type="hidden" name="action" value="acmebot_handle_form">
                        <?php wp_nonce_field('acmebot_settings_action', 'acmebot_settings_nonce'); ?>
                        <div class="acmebot-btn-center">
                            <?php submit_button(__('Connect to Acme Bot', 'acme-bot'), 'primary large acmebot-btn', 'submit', true); ?>
                        </div>
                    </form>
                <?php endif; ?>
            </div>
...
<?php
// Enqueue the CSS inline for simplicity
// In production, you might want to load this from a file
?>
<style>
/* Acme Bot - Enhanced Admin Settings
 * Improved styling for the Acme Bot WordPress plugin settings page
 */

/* Main Settings Container */
.settings-wrap {
    font-family: 'Segoe UI', 'Roboto', 'Arial', sans-serif;
    background: linear-gradient(145deg, #f8faff 0%, #f0f4fa 100%);
    min-height: calc(100vh - 10rem);
    padding: 40px 20px 60px;
    border-radius: 20px;
    box-shadow: 0 5px 30px rgba(30,34,90,0.08), 0 1.5px 8px rgba(30,34,90,0.05);
    margin-top: 20px;
}
...
/* Connect Button */
.acmebot-btn.button-primary.large {
    background: linear-gradient(90deg, #2563eb 0%, #22d3ee 100%);
    border: none;
    color: #fff;
    font-size: 1.15em;
...
.acmebot-btn.button-primary.large:hover,
.acmebot-btn.button-primary.large:focus {
    background: linear-gradient(90deg, #1e40af 0%, #06b6d4 100%);
    box-shadow: 0 6px 20px rgba(37, 99, 235, 0.35);
    transform: translateY(-2px);
}

.acmebot-btn.button-primary.large:active {
